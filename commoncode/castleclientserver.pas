    ...
    TCastleTCPClientThread = class(TThread)
      ...
      protected
        ...
        function ProcessMessage( const msgheader : TmsgHeader;
                                 Buffer : TIdBytes ) : boolean; virtual;

      public
        constructor Create(const AClient: TIdTCPClient;
                           const AOnMessageReceived, AOnConnected, AOnDisconnected: TProcedureObject); virtual;
        destructor Destroy; override;
      public
...
type
  ...
  public
    ...
    procedure SendBufferToClient(const Buffer : TIdbytes; ALength : integer; AClient: TClientConnection); virtual;
    procedure SendToAll(const AMessage: String); virtual;
  public
    function IsConnected: Boolean; overload; virtual;
    function Connections : integer;
  ...
  public
    ...
    protected
    function CreateClientThread : TCastleTCPClientThread; virtual;
  end;
...
{$ifndef ANDROID}
  constructor TCastleTCPClientThread.Create(const AClient: TIdTCPClient;
  ...
  function TCastleTCPClientThread.ProcessMessage( const msgheader : TmsgHeader;
  ...
  procedure TCastleTCPClientThread.ReadAndProcessMessage;
   var buffer : TIdBytes;
       msgheader : TMsgHeader;
   begin
     setlength( buffer, sizeof( msgheader ));
     FClient.IOHandler.ReadBytes( buffer, sizeof( msgheader ), false);
     move( buffer[0], msgheader, sizeof( msgheader ));
     ProcessMessage( msgheader, Buffer );
   end;
...
begin
  AClient.SendString( AMessage );
end;

procedure TCastleTCPServer.SendBufferToClient(const Buffer : TIdBytes; ALength : integer; AClient: TClientConnection);
 begin
   AClient.SendBuffer( Buffer, ALength );
 end;
...
begin
  ...
  {$else}
    for LContext in FClientConnectionsList.LockList do
     begin
       msgtype := 2;
       LContext.Connection.IOHandler.Write(msgtype);
...
function TCastleTCPServer.Connections : integer;
begin
   {$ifdef ANDROID}
     !!!Result := FTCPConnectionService.IsConnected;
   {$else}
     Result := FClientConnectionsList.LockList.Count;
     FClientConnectionsList.UnlockList;
   {$endif}
end;
...
function TCastleTCPClient.CreateClientThread : TCastleTCPClientThread;
 begin
