...
function UpdateTile( const tileparams : TTerrainParams;
 ...
 begin
   Result := GTileList.Initxy( TileX, TileY, GDefGridCellCount );
   Assert( assigned( tileparams ) and assigned( result ));
...
procedure buildArea( client : TClientConnection;
 ...
 procedure SendATile( tx, ty : integer; LOD : dword );
  var Tile : TTerTile;
  begin
    dbgwriteln( 'Send Tile['+inttostr( tx )+','+inttostr(ty)+','+inttostr(LOD)+']');
